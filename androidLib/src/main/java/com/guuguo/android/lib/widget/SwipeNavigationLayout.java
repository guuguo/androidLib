...
public class SwipeNavigationLayout extends FrameLayout {
    ...
    public SwipeNavigationLayout(@NonNull Context context, @Nullable AttributeSet attrs, @AttrRes int defStyleAttr) {
    ...
    private class SwipeGesture extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            if (showType == -1 && navigationView == null && Math.abs(distanceX / distanceY) > 2) {
                if (distanceX < 0) {
                    navigationView = LayoutInflater.from(getContext()).inflate(R.layout.layout_navigation_left, SwipeNavigationLayout.this, false);
                    showType = 1;
                } else {
                    navigationView = LayoutInflater.from(getContext()).inflate(R.layout.layout_navigation_right, SwipeNavigationLayout.this, false);
                    showType = 2;
                }
